...
inline std::size_t num_greater_pivots(std::int32_t const* pivots, int n,
                                      std::int32_t e) {
    ...
    if (n >= 32) {
        for (; pivots <= pivots_end - 32; pivots += 32) {
            __m256i const data1 = _mm256_loadu_si256((const __m256i*)(pivots));
            __m256i const data2 =
                _mm256_loadu_si256((const __m256i*)(pivots + 8));
            __m256i const data3 =
                _mm256_loadu_si256((const __m256i*)(pivots + 16));
            __m256i const data4 =
                _mm256_loadu_si256((const __m256i*)(pivots + 24));
    ...
    for (; pivots <= pivots_end - 8; pivots += 8) {
        __m256i const data = _mm256_loadu_si256((const __m256i*)(pivots));
...
#endif
            count +=
                _mm_popcnt_u32(_mm256_movemask_ps(_mm256_castsi256_ps(cmp)));
#if defined SKIP_AVX2_POPCNT
            return count;
        }
#endif
...
inline std::size_t find_bucket(std::int32_t const* pivots, int n,
                               std::int32_t e) {
...
class QuickHeapAVX2 {
   ...
   private:
    ...
    void partition_last_bucket() {
        ...
        buckets_[num_buckets_ + 1].resize((n + 7) & ~7);
        buckets_[num_buckets_].resize((n + 7) & ~7);
        buckets_[num_buckets_ - 1].resize((n + 7) & ~7);
        auto const data = buckets_[num_buckets_ - 1].data();
        ...
        for (int i = 0; i <= static_cast<int>(n) - 8; i += 8) {
            __m256i const v = _mm256_loadu_si256((const __m256i*)(data + i));
            auto const mask_smaller = static_cast<unsigned>(_mm256_movemask_ps(
                _mm256_castsi256_ps(_mm256_cmpgt_epi32(pivot_vec, v))));
            {  // Smaller
                auto const mask_expanded =
                    _pdep_u64(mask_smaller, 0x0101010101010101) * 0xFF;
                auto const indices =
                    _pext_u64(0x0706050403020100, mask_expanded);
                __m256i const shuffled = _mm256_permutevar8x32_epi32(
                    v, _mm256_cvtepu8_epi32(_mm_cvtsi64_si128(indices)));
                auto const count = _mm_popcnt_u32(static_cast<unsigned>(mask_smaller));
                _mm256_storeu_si256((__m256i*)smaller_ptr, shuffled);
                smaller_ptr += count;
            }
            ...
            {  // Greater
                auto const mask_expanded =
                    _pdep_u64(mask_greater, 0x0101010101010101) * 0xFF;
                auto const indices =
                    _pext_u64(0x0706050403020100, mask_expanded);
                __m256i const shuffled = _mm256_permutevar8x32_epi32(
                    v, _mm256_cvtepu8_epi32(_mm_cvtsi64_si128(indices)));
                auto const count = _mm_popcnt_u32(static_cast<unsigned>(mask_greater));
                _mm256_storeu_si256((__m256i*)greater_ptr, shuffled);
                greater_ptr += count;
            }
            auto const mask_equal = ~(mask_smaller | mask_greater) & 0xFF;
            {  // Equal
                auto const mask_expanded =
                    _pdep_u64(mask_equal, 0x0101010101010101) * 0xFF;
                auto const indices =
                    _pext_u64(0x0706050403020100, mask_expanded);
                __m256i const shuffled = _mm256_permutevar8x32_epi32(
                    v, _mm256_cvtepu8_epi32(_mm_cvtsi64_si128(indices)));
                auto const count = _mm_popcnt_u32(static_cast<unsigned>(mask_equal));
                _mm256_storeu_si256((__m256i*)equal_ptr, shuffled);
                equal_ptr += count;
            }
        }
        auto const remaining = n % 8;
        if (remaining != 0) {
            unsigned const valid_mask = (1 << remaining) - 1;
            __m256i const v =
                _mm256_loadu_si256((const __m256i*)(data + n - remaining));
            auto const mask_smaller = static_cast<unsigned>(_mm256_movemask_ps(_mm256_castsi256_ps(
                                          _mm256_cmpgt_epi32(pivot_vec, v)))) &
                                      valid_mask;
            auto const mask_greater = static_cast<unsigned>(_mm256_movemask_ps(_mm256_castsi256_ps(
                                          _mm256_cmpgt_epi32(v, pivot_vec)))) &
                                      valid_mask;
        ...
        buckets_[num_buckets_ + 1].resize(
            static_cast<std::size_t>(std::distance(buckets_[num_buckets_ + 1].data(), smaller_ptr)));
        buckets_[num_buckets_].resize(
            static_cast<std::size_t>(std::distance(buckets_[num_buckets_].data(), equal_ptr)));
        buckets_[num_buckets_ - 1].resize(
            static_cast<std::size_t>(std::distance(buckets_[num_buckets_ - 1].data(), greater_ptr)));
   ...
   public:
    explicit QuickHeapAVX2() : buckets_(128), pivots_(128) {}

    void push(T e) {
        assert(buckets_.size() >= (num_buckets_ | 1));
        assert((pivots_.size() & 7) == 0);
        auto const b = quick_heap_avx2_detail::find_bucket(pivots_.data(),
                                                           num_buckets_ / 2, e);
        assert(b < (num_buckets_ | 1));
        auto& bucket = buckets_[b];
        bucket.push_back(e);
        if (bucket.capacity() < ((bucket.size() + 7) & ~7)) {
            auto const old_size = bucket.size();
            bucket.insert(bucket.end(), 8 - (bucket.size() & 7), 0);
    ...
    void pop() {
        ...
        if ((num_buckets_ & 1) == 1 && !buckets_[num_buckets_ - 1].empty()) {
            auto m = quick_heap_avx2_detail::find_min(
